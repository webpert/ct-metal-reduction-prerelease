...
from r2_gaussian.dataset import Scene, Projector
...
def training(
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # if torch.isnan(loss["total"]).any() or torch.isinf(loss["total"]).any():
...
def training_report(
    ...
    if iteration in testing_iterations:
        ...
        vol_pred_main = vol_tot["vol"]
        vol_pred_sub = vol_tot["vol_res"]
        vol_pred = vol_tot["vol"] + (
            vol_tot["vol_res"] * scene.projector.get_optimal_weight_for_b.item()
        )
        vol_gt = scene.vol_gt
        vol_mask = scene.vol_mask
        G = torch.where(vol_mask > 0.9, 0, vol_gt)
        V = torch.where(vol_mask > 0.9, 0, vol_pred)
        ...
        if tb_writer:
            vmax = np.percentile(G.cpu().detach().numpy(), scene.view_percentile)
            image_show_3d = np.concatenate(
                [
                    ...
                    for i in np.linspace(0, G.shape[2], 7).astype(int)[1:-1]
                ],
                axis=0,
            )
