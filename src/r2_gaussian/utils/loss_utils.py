...
def grad_l2_3d(volume: torch.Tensor) -> torch.Tensor:
    """
    Compute L2 gradient regularization for a 3D volume.
    
    Args:
        volume (torch.Tensor): 3D tensor of shape (D, H, W) or (1, D, H, W)
    ...
    reg = (dz**2).mean() + (dy**2).mean() + (dx**2).mean()
    return reg
...
def ncc_loss(x, y, reduction='mean', eps=1e-8):
