...
def show_two_slice(
    ...
    if not no_diff:
        ...
        img3 = ax[2].imshow(diff, cmap='bwr', vmin=-np.max(np.abs(diff)), vmax=np.max(np.abs(diff)))    # kschoi        ax[2].title.set_text("error")
...
def show_two_volume_ab(
    ...
    volume12 = (volume1 + volume2) ** gamma
    ...
    if title2 is not None:
        ax[1].title.set_text(title2)
    img3 = ax[2].imshow(
        volume12[:, :, initial_slice], cmap=cmap, vmin=0.0, vmax=volume12.max()
    )
    ax[2].title.set_text("error")
    ...
    def update(val):
        ...
        img1.set_array(volume1[:, :, slice_index])
        img2.set_array(volume2[:, :, slice_index])
        img3.set_array(volume12[:, :, slice_index])
        fig.canvas.draw_idle()
