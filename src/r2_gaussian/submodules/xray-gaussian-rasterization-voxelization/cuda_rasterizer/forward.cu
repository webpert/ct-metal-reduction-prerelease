...
__global__ void preprocessCUDA(int P,
	...
	// output arguments from here, kschoi
	int* radii,
	...
	)
{
	...
	float3 conic = { cov.z * det_inv, -cov.y * det_inv, cov.x * det_inv };	// kschoi, inverse matrix of cov (2D)
	...
	opacity_res[idx] = opacities_res[idx];
	tiles_touched[idx] = (rect_max.y - rect_min.y) * (rect_max.x - rect_min.x);
	mus[idx] = cov.w;	// kschoi, compensation factor 
}
...
renderCUDA(
	...
	float* __restrict__ out_B_k,
	float* __restrict__ out_C_k,
	...
	)
{
	...
	float sum_opacity = 0.0;
	float sum_opacity_res = 0.0;	// kschoi
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			collected_opacity_res[block.thread_rank()] = opacity_res[coll_id];
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float o_res = collected_opacity_res[j];		// kschoi
			...
			const float alpha_res = o_res * mu * exp(power);	// kschoi
			...
			sum_opacity += alpha;
			sum_opacity_res += alpha_res;	// kschoi
	...
	if (inside)
	{
		// TODO: use sum_opacity_res[0]
		n_contrib[pix_id] = last_contributor;

		const float N = static_cast<float>(BHC_ETA_COUNT);
		const float a = sum_opacity;		
		const float b = sum_opacity_res;
		float eps = 1e-35f;
		float A = 0.0;
		float B_k = 0.0;
		float C_k = 0.0;
		...
		{
			C_k = (-1.0) / (((2.0 * k)/(N - 1.0) - 1.0) * bhc_gamma[0] + 1.0);
			C_k = C_k * C_k * C_k; 	// pow(C_k, 3.0)
			exponent = C_k * b;
			B_k = exp(exponent);
			A += (bhc_eta[k] * B_k);
			out_B_k[k * H * W + pix_id] = B_k;
			out_C_k[k * H * W + pix_id] = C_k;
			debug = fminf(debug, exponent);
		}
		A = fmaxf(A, eps);		// to avoid exception
		out_color[pix_id] = a - log(A);
		out_A[pix_id] = A;
		out_D[pix_id] = exponent;		// debug port
	}
}
