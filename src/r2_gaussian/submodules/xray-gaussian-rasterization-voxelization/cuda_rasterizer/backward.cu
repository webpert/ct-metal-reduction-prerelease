...
renderCUDA(
	...
	int W, int H,
	const float* __restrict__ bhc_gamma,
	const float* __restrict__ bhc_eta,
	const float2* __restrict__ points_xy_image,
	const float4* __restrict__ conic_opacity,
	const float* __restrict__ opacity_res,
	const float* __restrict__ mus,
	const uint32_t* __restrict__ n_contrib,
	const float* __restrict__ dL_dpixels,
	const float* __restrict__ A,	// kschoi, [1, H, W]
	const float* __restrict__ B_k,	// kschoi, [BHC_ETA_COUNT, H, W]
	const float* __restrict__ C_k,	// kschoi, [BHC_ETA_COUNT, H, W]
	const float* __restrict__ D,	// kschoi, [1, H, W]
	float3* __restrict__ dL_dmean2D,
	float4* __restrict__ dL_dconic2D,
	float* __restrict__ dL_dopacity,
	float* __restrict__ dL_dopacity_res,
	float* __restrict__ dL_dmu,
	float* __restrict__ dL_dbhc_eta,
	float* __restrict__ dL_dbhc_eta_pixel,
	float* __restrict__ dL_dbhc_gamma,
	float* __restrict__ dL_dbhc_gamma_pixel)
{	
	...
	__shared__ float collected_opacity_res[BLOCK_SIZE];
	...
	float dL_dpixel;
	float A_pixel;
	float B_pixel[BHC_ETA_COUNT];
	float C_pixel[BHC_ETA_COUNT];	
	float D_pixel;
	if (inside){
		...
		{
			B_pixel[k] = B_k[k * H * W + pix_id];
			C_pixel[k] = C_k[k * H * W + pix_id];
		}
		D_pixel = D[pix_id];	// TODO: dummy buffer, unused
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			collected_opacity_res[block.thread_rank()] = opacity_res[coll_id];	// kschoi
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float o_res = collected_opacity_res[j];	// TODO: use this value
			...
			const float s = mu;
			const float a= o_res;
			const float b = con_o.w;

			const float alpha = b * mu * G;
			if (alpha <0.00001f)	// TODO, should be checked if necessary
				continue;
			...
			float dL_dalpha = dL_dpixel;		// dL_dI
			...
			const float N = static_cast<float>(BHC_ETA_COUNT);
			const float aG = a * G;
			const float bG = b * G;
			const float as = a * s;
			const float bs = b * s;
			const float sG = s * G;
			float B_k_C_k = 0.0f;
			float minus_inverse_A;
			if (abs(A_pixel) >= 1e-35f)
				minus_inverse_A = (-1.0f)/(A_pixel);
			else
				minus_inverse_A = -1e+35f;

			for (int k=0; k<BHC_ETA_COUNT; k++)
			{	
				const float B_k = B_pixel[k];
				const float C_k = C_pixel[k];
				B_k_C_k += (bhc_eta[k] * B_k * C_k);
			}

			const float coeff_common = minus_inverse_A * B_k_C_k;
			const float dL_da = dL_dalpha * sG;
			const float dL_db = dL_dalpha * coeff_common * sG;
			const float dL_ds = dL_dalpha * (aG + (coeff_common * bG));
			const float dL_dG = dL_dalpha * (as + (coeff_common * bs));

			// kschoi, rho: con_o.w, mu: mu, G: exp(..)
			...
			atomicAdd(&(dL_dopacity[global_id]), dL_da);	// dL_da
			atomicAdd(&(dL_dopacity_res[global_id]), dL_db);	// dL_db
			atomicAdd(&(dL_dmu[global_id]), dL_ds);	// dL_ds			

			for (int k=0; k<BHC_ETA_COUNT; k++)
			{
				dL_dbhc_eta_pixel[k*H*W + pix_id] = dL_dalpha * minus_inverse_A * B_pixel[k];
			}
			// dL_dbhc_gamma_pixel[pix_id] = 0.0f;		// dummy buffer		
		}
	}
}
...
__global__ void accumulate_bhc_eta(float* dL_dbhc_eta, float* dL_dbhc_eta_pixel, int W, int H) {
    ...
    for (int h = 0; h < H; h++) {
        for (int w = 0; w < W; w++) {
            sum += dL_dbhc_eta_pixel[k * W * H + W * h + w];
        }
    }
    dL_dbhc_eta[k] = sum;
}

__global__ void accumulate_bhc_gamma(float* dL_dbhc_gamma, float* dL_dbhc_gamma_pixel, int W, int H) {
	...
	)
{
    ...
    accumulate_bhc_eta<<<1, BHC_ETA_COUNT>>>(dL_dbhc_eta, dL_dbhc_eta_pixel, W, H);
    // accumulate_bhc_gamma<<<1, 1>>>(dL_dbhc_gamma, dL_dbhc_gamma_pixel, W, H);
	cudaDeviceSynchronize();

    cudaFree(dL_dbhc_eta_pixel);	
	// cudaFree(dL_dbhc_gamma_pixel);
}