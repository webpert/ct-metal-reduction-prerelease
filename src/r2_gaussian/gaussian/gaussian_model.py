...
class GaussianModel:
    def setup_functions(self):
        ...
        self.density_res_activation = torch.nn.Softplus()   # it can be minus value
    ...
    def create_from_pcd(self, xyz, density, spatial_lr_scale: float, bhc_density_init_scale: float = 1.0):
        ...
        fused_density_res = (
            self.density_res_inverse_activation(torch.tensor(density*0.5) * bhc_density_init_scale).float().cuda()
        )
    ...
    def reset_density(self, reset_density=1.0):
        ...
        densities_res_new = self.density_res_inverse_activation(
            torch.min(
                self.get_density_res, torch.ones_like(self.get_density) * reset_density * 0.5 # TODO, kschoi
            )
        ) 
    ...
    def densify_and_prune(
        ...
        scaled_max_grad = max_grad * 100.0
