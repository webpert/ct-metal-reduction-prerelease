...
def readBlenderInfo(path, eval):
    ...
    path_vol_mask = osp.join(osp.dirname(meta_data["vol"]), "vol_mask.npy")
    if osp.isfile(path_vol_mask):
        vol_mask = torch.from_numpy(np.load(path_vol_mask)).float().cuda()
    else:
        vol_mask = torch.zeros_like(vol_gt).float().cuda()
...
def readCTameras(meta_data, source_path, eval=False, scene_scale=1.0):
    ...
    for split in splits:
        ...
        for i_split in range(n_split):
            ...
            image = np.load(image_path.replace("\\","/")) * scene_scale
