...
from r2_gaussian.arguments import ParamGroup, ModelParams, PipelineParams, OptimizationParams
...
def init_pcd(
    ...
    if recon_method == "random":
        ...
        print(f"Initialize point clouds from {recon_method} reconstruction.")
        vol = None
        if recon_method == "fdk":
            vol_fdk_name = osp.join(osp.dirname(save_path), "vol_fdk.npy")
            if osp.isfile(vol_fdk_name):
                print(f"Use cached reconstruction: {vol_fdk_name}")
                vol = np.load(vol_fdk_name)

        if vol is None:
            vol = recon_volume(projs, angles, copy.deepcopy(geo), recon_method)
            if recon_method == "fdk":
                print(f"Cache reconstruction to {vol_fdk_name}")
                np.save(vol_fdk_name, vol)
...
def main(
    args, init_args: InitParams, model_args: ModelParams, opt_args: OptimizationParams, pipe_args: PipelineParams
):
    # Read scene
    data_path = args.data
    ...
    if args.evaluate:
        with torch.no_grad():
            ...
            initialize_gaussian(gaussians, model_args, opt_args, None)
            vol_pred = query(
                gaussians,
                None,
...
if __name__ == "__main__":
    ...
    main(args, init_parser.extract(args), lp.extract(args), op.extract(args), pp.extract(args))
